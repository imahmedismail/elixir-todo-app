defmodule ElixirTodoAppWeb.ItemControllerTest do
  ...
  describe "create/2 action of item-controller" do
    test "creates the todo-item", %{conn: conn} do
      {:ok, list} = Repo.insert(%List{title: "List 1"})
      response = ItemController.create(conn, %{"list_id" => list.id, "item" => %{"content" => "Item1", "todo_list_id" => list.id}}).resp_body |> Jason.decode!()
      assert response["item"]["content"] == "Item1"
    end

    test "with empty-params throws unidentified error", %{conn: conn} do
      {:ok, list} = Repo.insert(%List{title: "List 1"})
      response = ItemController.create(conn, %{"list_id" => list.id, "item" => %{}}).resp_body |> Jason.decode!()
    ...
    test "throws error if invalid list-id is passed", %{conn: conn} do
      ...
      response = ItemController.create(conn, %{"list_id" => @invalid_id, "item" => %{}}).resp_body |> Jason.decode!()
    ...
    test "can't create an item in archived list", %{conn: conn} do
      ...
      response = ItemController.create(conn, %{"list_id" => list.id, "item" => %{"content" => "test", "todo_list_id" => list.id}}).resp_body |> Jason.decode!()
  ...
  describe "update/2 action of item-controller" do
    test "updates the item", %{conn: conn} do
      ...
      response = ItemController.update(conn, %{"list_id" => list.id, "id" => item.id, "item" => %{"content" => "Updated Content"}}).resp_body |> Jason.decode!()
    ...
    test "with empty params do nothing", %{conn: conn} do
      {:ok, list} = Repo.insert(%List{title: "List 1"})
      {:ok, item} = Repo.insert(%Item{content: "List 1", todo_list_id: list.id})
      response = ItemController.update(conn, %{"list_id" => list.id, "id" => item.id, "item" => %{}}).resp_body |> Jason.decode!()
    ...
    test "with invalid list-id returns error", %{conn: conn} do
      {:ok, list} = Repo.insert(%List{title: "List 1"})
      {:ok, item} = Repo.insert(%Item{content: "List 1", todo_list_id: list.id})
      response = ItemController.update(conn, %{"list_id" => @invalid_id, "id" => item.id, "item" => %{}}).resp_body |> Jason.decode!()
      assert response["message"] == "No such list exists"
    end
    ...
    test "with invalid item-id returns error", %{conn: conn} do
      ...
      response = ItemController.update(conn, %{"list_id" => list.id, "id" => @invalid_id, "item" => %{}}).resp_body |> Jason.decode!()
    ...
    test "can't update the item-content of archived list", %{conn: conn} do
      {:ok, list} = Repo.insert(%List{title: "List 1", archived: true})
      {:ok, item} = Repo.insert(%Item{content: "List 1", todo_list_id: list.id})
      response = ItemController.update(conn, %{"list_id" => list.id, "id" => item.id, "item" => %{"content" => "trying to update"}}).resp_body |> Jason.decode!()
