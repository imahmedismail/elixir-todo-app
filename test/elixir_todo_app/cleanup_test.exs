defmodule ElixirTodoApp.CleanupTest do
  ...
  setup do
    updated_at = Timex.shift(Timex.now(), days: -2) |> Timex.to_naive_datetime() |> NaiveDateTime.truncate(:second)
    inserted_at = NaiveDateTime.utc_now() |> NaiveDateTime.truncate(:second)

    todo_lists = [
      %{id: 1, title: "List 1", archived: false, inserted_at: inserted_at, updated_at: updated_at},
      %{id: 2, title: "List 2", archived: false, inserted_at: inserted_at, updated_at: inserted_at},
  ...
  test "todo-cleanup archives unarchived Todo lists not updated in the last 24 hours", %{todo_lists: todo_lists} do
    mock_cleanup_functionality()
    assert todo_lists != ElixirTodoApp.Repo.all(ElixirTodoApp.List)
    assert Enum.filter(ElixirTodoApp.Repo.all(ElixirTodoApp.List), fn list -> list.archived == true end) |> Enum.count() == 3
  end
